...
// class constructor with pulling force initialization
RampMinimizerPulling::RampMinimizerPulling(const Vector3& pulling_force) :
RampMinimizer(),
m_pulling_force(pulling_force) {};


// minimization method
bool RampMinimizerPulling::binding_ramp_minimization() {
    ...
    for (Size i=0; i<m_binding_ramp_sampling; ++i) {
        ...
        bp_coll_intf_ptr->set_pulling_force(m_pulling_force);
        ...
        // energy contribs
        const std::vector<EnergyContrib> contribs =
        bp_coll_intf_ptr->energy_contribs();
        ...
        for (const EnergyContrib& c : contribs) {
            data.push_back(c.first);
            data.push_back(EnhancedString::convert_to_string(c.second));
        };
        log_stats(data);
...
const Vector3& RampMinimizerPulling::pulling_force() const {
