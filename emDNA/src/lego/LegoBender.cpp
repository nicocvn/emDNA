...
LegoBender::LegoBender(const Size& n_bp,
                       const Real& bending_angle) :
LegoProtein(), m_n_bp(n_bp), m_bending_angle(bending_angle) {
    compute_bp_collection();
};


// step parameters computation method
void LegoBender::compute_bp_collection() {

    // IdealDNA force field
    // this force field is uniform so we can use dummy sequence
    StepParametersDB model;
    model.init_with_model("IdealDNA");
    ...
    for (Size i=0; i<n_steps; ++i) {
        VectorN pvec = {
            m_bending_angle*std::sin(i*tw0),
            m_bending_angle*std::cos(i*tw0),
            FLOAT_INIT,
            FLOAT_INIT,
            FLOAT_INIT,
            FLOAT_INIT
